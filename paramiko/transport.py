...
class Transport (threading.Thread):
    ...
    def send_ignore(self, bytes=None):
        ...
        """
        m = Message()
        m.add_byte(MSG_IGNORE)
        randpool.stir()
        if bytes is None:
            bytes = (randpool.get_bytes(1) % 32) + 10
        ...
        try:
            ...
            while self.active:
                ...
                if ptype in self._handler_table:
                    ...
                    msg = Message()
                    msg.add_byte(MSG_UNIMPLEMENTED)
                    msg.add_int(m.seqno)
    ...
    def _parse_kex_init(self, m):
        ...
        self.remote_kex_init = bytes((MSG_KEXINIT,)) + m.get_so_far()
    ...
    def _parse_global_request(self, m):
        ...
        if want_reply:
            msg = Message()
            if ok:
                msg.add_byte(MSG_REQUEST_SUCCESS)
                msg.add(*extra)
            else:
                msg.add_byte(MSG_REQUEST_FAILURE)
            self._send_message(msg)
    ...
    def _parse_channel_open(self, m):
        ...
        if reject:
            msg = Message()
            msg.add_byte(MSG_CHANNEL_OPEN_FAILURE)
            msg.add_int(chanid)
            msg.add_int(reason)
            msg.add_string('')
            msg.add_string('en')
        ...
        try:
            ...
            self.lock.release()
        m = Message()
        m.add_byte(MSG_CHANNEL_OPEN_SUCCESS)
        m.add_int(chanid)
        m.add_int(my_chanid)
        m.add_int(self.window_size)
        m.add_int(self.max_packet_size)
