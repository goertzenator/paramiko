...
class Channel (object):
    ...
    def get_pty(self, term='vt100', width=80, height=24):
        ...
        m = Message()
        m.add_byte(MSG_CHANNEL_REQUEST)
    ...
    def recv(self, nbytes):
        ...
        if ack > 0:
            m = Message()
            m.add_byte(MSG_CHANNEL_WINDOW_ADJUST)
            m.add_int(self.remote_chanid)
            m.add_int(ack)
            self.transport._send_user_message(m)
    ...
    def send_stderr(self, s):
        ...
        try:
            ...
            if size == 0:
                ...
                return 0
            m = Message()
            m.add_byte(MSG_CHANNEL_EXTENDED_DATA)
            m.add_int(self.remote_chanid)
            m.add_int(1)
            m.add_string(s[:size])
        finally:
    ...
    def _handle_request(self, m):
        ...
        if want_reply:
            m = Message()
            if ok:
                m.add_byte(MSG_CHANNEL_SUCCESS)
            else:
                m.add_byte(MSG_CHANNEL_FAILURE)
            m.add_int(self.remote_chanid)
            self.transport._send_user_message(m)
