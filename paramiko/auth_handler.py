...
class AuthHandler (object):
    ...
    def _request_auth(self):
        m = Message()
        m.add_byte(MSG_SERVICE_REQUEST)
        m.add_string('ssh-userauth')
        self.transport._send_message(m)

    def _disconnect_service_not_available(self):
        m = Message()
        m.add_byte(MSG_DISCONNECT)
    ...
    def _get_session_blob(self, key, service, username):
        ...
        m.add_byte(MSG_USERAUTH_REQUEST)
    ...
    def _send_auth_result(self, username, method, result):
        ...
        if result == AUTH_SUCCESSFUL:
            ...
            m.add_byte(MSG_USERAUTH_SUCCESS)
            self.authenticated = True
        else:
            self.transport._log(INFO, 'Auth rejected (%s).' % method)
            m.add_byte(MSG_USERAUTH_FAILURE)
    ...
    def _interactive_query(self, q):
        ...
        m = Message()
        m.add_byte(MSG_USERAUTH_INFO_REQUEST)
        m.add_string(q.name)
    ...
    def _parse_userauth_info_request(self, m):
        ...
        m = Message()
        m.add_byte(MSG_USERAUTH_INFO_RESPONSE)
